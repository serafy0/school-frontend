  ...
  <div>
    <section v-if="authenticated && user.role === 'PARENT'">
      <div class="container is-centered is-fluid">
        ...
        <b-loading v-model="loading" is-full-page :can-cancel="true"></b-loading>
      </div>
    ...
    <section v-if="authenticated && user.role === 'PARENT'" class="pt-6">
    ...
    <section v-else>
      <b-message type="is-danger">
        this section is reserved for parents maybe you need to
        <nuxt-link to="/login" class="has-text-info">login</nuxt-link>
      </b-message>
    </section>
  </div>
</template>
  ...
  methods: {
    ...
    async signUp() {
      ...
      await this.$axios
        ...
        .then((res) => {
          this.$buefy.toast.open(`you just added your child, say hi to  ${this.form.first_name} !`)
          this.fetchSomething()
          // this.sendLogin()
        })
        .catch((e) => {
